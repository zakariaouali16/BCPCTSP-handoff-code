...
public class Main {
    ...
    static String begin = "";
    static String end = "";
    static double budget = 1500000; // Provide a default coordinate budget here
    static int n = 48;
    ...
    static String gaOnePrize = "";
    static String gaTwoPrize = "";
    static String gaOneDist = "";
    static String gaTwoDist = "";
    static StringBuilder prizeCol = new StringBuilder("");

    public static void main(String[] args) throws IOException {
        long startTime, endTime;
        double totalTime;

        // Load city list once so we can pick random start/end each run
        initList();
        ...
        for (int run = 1; run <= 20; run++) {
            ...
            int startIdx = rand.nextInt(arrCities.size());
            begin = arrCities.get(startIdx).name;

            // Pick a random end city different from start
            int endIdx;
            do {
                endIdx = rand.nextInt(arrCities.size());
            } while (endIdx == startIdx);
            end = arrCities.get(endIdx).name;

            System.out.println("--- Random Run Parameters ---");
            System.out.println("Selected Start City ID: " + begin);
            ...
            for (double b : budgets) {
                ...
                totalTime = (double) (endTime - startTime) / 1000000;
                System.out.println("Algorithm took " + totalTime + "ms to process.");
                System.out.printf("\nTotal distance: %6.2f Miles", AntQ.finalDist);
                System.out.printf("\nCollected Prize: $%d", AntQ.finalPrize - arrCities.get(0).pop * 2);
                System.out.printf("\nRoute: %s\n", AntQ.finalRoute.toString());
                System.out.printf("Remaining Budget: %.2f Miles\n", AntQ.remainingBudget);
                System.out.println("Best episode prize (P^m): " + (AntQ.bestRoutePrize - arrCities.get(0).pop));
                System.out.println("Found on episode: " + AntQ.bestRouteIter);
            }
        }
    }
    ...
    static void initList(boolean flag) {
        ...
        try {
            ...
            // (2)
            if (scan.hasNextLine()) {
                scan.nextLine(); // Consumes the "city_id,x,y,reward" line
            }
    ...
    static void initList() {
        ...
        // (3)/(4) - only restructure once begin/end are set
